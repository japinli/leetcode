/*----------------------------------------------------------------------------
 *
 * robot_return_to_origin.c
 *   There is a robot starting at position `(0, 0)`, the origin, on a 2D
 *   plane. Given a sequence of its moves, judge if this robot
 *   __ends up at (0, 0)__ after it completes its moves.
 *
 *   The move sequence is represented by a string, and the character
 *   `moves[i]` represents its ith move. Valid moves are `R(right)`,
 *   `L(left)`, `U(up)`, and `D(down)`. If the robot returns to the
 *   origin after it finishes all of its moves, return `true`.
 *   Otherwise, return `false`.
 *
 *   __Note:__ The way that the robot is "facing" is irrelevant. `"R"`
 *   will always make the robot move to the right once, `"L"` will
 *   always make it move left, etc. Also, assume that the magnitude of
 *   the robot's movement is the same for each move.
 *
 *----------------------------------------------------------------------------
 */
#include <stdbool.h>

bool
judgeCircle1(char *moves)
{
    int x = 0;
    int y = 0;

    while (*moves) {
        switch (*moves) {
        case 'L':
            x--;
            break;
        case 'R':
            x++;
            break;
        case 'U':
            y++;
            break;
        case 'D':
            y--;
            break;
        default:
            break;
        }
        moves++;
    }

    return x == 0 && y == 0;
}


bool
judgeCircle(char *moves) {

    int x = 0;
    int y = 0;

    while (*moves) {
        if (*moves == 'L') {
            x--;
        } else if (*moves == 'R') {
            x++;
        } else if (*moves == 'U') {
            y++;
        } else if (*moves == 'D') {
            y--;
        }
        moves++;
    }

    return x == 0 && y == 0;
}
